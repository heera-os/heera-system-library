...
{
    QVariantMap temp = m_currentController->metadata();
    
    if (temp["mpris:artUrl"].toString().startsWith("https://open.spotify.com/image")) 
        temp["mpris:artUrl"] = temp["mpris:artUrl"].toString().replace("https://open.spotify.com/image", "http://i.scdn.co/image");

    return checkController(Q_FUNC_INFO) ? temp : QVariantMap();
}
